...
import { BrainCircuit, Mail, Lock, User, AlertCircle, Coins } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center px-6 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md rounded-2xl border border-slate-800/80 bg-slate-900/40 p-8 shadow-2xl backdrop-blur-xl relative z-10">
        <div className="text-center mb-6">
          ...
          <p className="text-[11px] text-slate-500 mt-1 font-semibold">Join us to automate and predict your spending flow</p>
        </div>
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full pl-11 pr-4 py-3 rounded-xl border border-slate-800 bg-slate-950/80 text-sm text-slate-200 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 focus:outline-none transition-all duration-200 appearance-none cursor-pointer"
              >
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 mt-2 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-500 hover:scale-[1.01] hover:brightness-110 active:scale-95 text-sm font-bold text-white transition-all duration-150 shadow-md glow-cyan/10 flex items-center justify-center gap-2"
          >
            {loading ? (
        ...
        <p className="mt-6 text-center text-xs text-slate-500 font-medium">
          Already registered?{' '}
          <Link to="/login" className="text-cyan-400 hover:underline font-bold pl-0.5">
            Log In Here
          </Link>
        </p>
      </div>
